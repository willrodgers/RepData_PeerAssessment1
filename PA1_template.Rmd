...
ggplot(steps_by_day, aes(x=steps)) + geom_histogram(binwidth=2000, color="white") +
  scale_y_continuous(name="# of Days") +
  scale_x_continuous(name="# of Steps per Day", labels=comma, limits=c(0, 25000)) + 
...
```{r}
steps_by_interval <- ddply(data, c("interval"), summarise, avg_steps=mean(steps, na.rm=TRUE))
ggplot(steps_by_interval, aes(x=interval, y=avg_steps, group=1)) + geom_line(size=1) + 
  scale_x_discrete(name="Interval", breaks=paste(sprintf("%02d", seq(0,24,4)),"00",sep=":")) +
...
ggplot(steps_by_day, aes(x=steps)) + geom_histogram(binwidth=2000, color="white") +
  scale_y_continuous(name="# of Days") +
  scale_x_continuous(name="# of Steps per Day", labels=comma, limits=c(0, 25000)) + 
  ggtitle("Histogram of Total Steps per Day - Using Imputed Data") + 
  theme(plot.title = element_text(lineheight=0.8, face="bold"))
```
...
data_imputed$weekday_type <- as.factor(ifelse(data_imputed$weekday %in% c("Sunday", "Saturday"), 
  "weekend", "weekday"))
```
...
library(ggplot2)
library(scales)
steps_by_interval <- ddply(data_imputed, c("interval", "weekday_type"), summarise, avg_steps=mean(steps, na.rm=TRUE))
ggplot(steps_by_interval, aes(x=interval, y=avg_steps, group=weekday_type, color=weekday_type)) + 
  geom_line(size=1) + 
